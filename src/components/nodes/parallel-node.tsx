...
export function ParallelNode({ data }: any) {
  return (
    <div className="flex items-center border rounded-md bg-white w-48 shadow-sm">
      ...
      <div className="flex items-center justify-center w-10 border-r">
        <SquareStack className="h-5 w-5 text-blue-500" />
      </div>

      {/* Text section */}
      <div className="flex flex-col flex-1 px-2 py-1">
        <span className="text-xs text-gray-500">Parallel state</span>
        <span className="text-sm font-semibold">{data.label || "Parallel"}</span>
      </div>
